...
(function($){
	...
	GRVE.documentReady = {
		init: function(){
			...
			if( $('#grve-feature-section').length > 0 ){
				...
				GRVE.featureParallax.init( '#grve-feature-section' );
			}
			if( $('.grve-page-title').length > 0 ){
				GRVE.featureSection.init({ section : '.grve-page-title' });
				GRVE.featureSize.init( '.grve-page-title' );
			}
			GRVE.parallaxSection.init();
	...
	// ============================================================================= //
		...
		init: function(){
			...
			GRVE.pageSettings.sideMenuHeight();
	...
	GRVE.mainMenu = {
		init: function(){
			...
			var $mainMenu     = $('#grve-main-menu'),
				$menuItem     = $mainMenu.find('li.menu-item-has-children'),
				$megaMenuItem = $mainMenu.find('li.megamenu'),
				$subMenu      = $menuItem.find('ul.sub-menu'),
				windowWidth   = $(window).width() + scrollBarWidth;

			if( $mainMenu.hasClass('grve-arrow') ) {
				addArrows();
			}
			deviceMenu();


			$menuItem.on('mouseenter touchstart touchend',function(){
				var $item = $(this);
				$item.find('ul.right').removeClass('right');
				menuPosition( $item );
			});

			$('.grve-menu-no-link > a').on('click',function(e){
				e.preventDefault();
			});

			function menuPosition( $menuItem ) {
				var windowWidth  = $(window).width(),
					$nextSubMenu = $menuItem.find(' > ul.sub-menu'),
					offsetLeft   = $nextSubMenu.offset().left,
					itemWidth    = $nextSubMenu.outerWidth();

				if( windowWidth < offsetLeft + itemWidth ) {
					$nextSubMenu.addClass('right');
				} else {
					$nextSubMenu.removeClass('right');
				}
			}

			// Add Arrows
			function addArrows() {
				var $arrow = $('<i class="grve-arrow"></i>');
				$arrow.appendTo( $menuItem.find('.grve-item:first') );
			}

			// Device Menu
			function deviceMenu(){
				if( isMobile.any() && windowWidth > tabletPortrait ) {

					$menuItem.find('> a').bind('touchstart touchend', function(e) {
						var $item = $(this);
						$item.parent().siblings().removeClass('open-submenu');
						if( $item.attr('href') != '#' || $item.attr('href') === '#' ) {
							if( !$item.parent().hasClass('open-submenu') ) {
								e.preventDefault();
								$item.parent().addClass('open-submenu');
							}
						}
					...
					$(document).bind('touchstart touchend', function(e) {
						if ( !$menuItem.is(e.target) && $menuItem.has(e.target).length === 0 ) {
							$menuItem.removeClass('open-submenu').find('li').removeClass('open-submenu');
						}
					});
	...
	GRVE.featureParallax = {
		init: function( section ){
			...
			if( !$( section ).hasClass('grve-bg-parallax') ) return;

			var $section        = $( section ),
				$parallaxEl     = $section.find('.grve-bg-image, .grve-bg-video'),
				$content        = $section.find('.grve-content'),
				elementHeight   = GRVE.featureSize.init( section ),
				parallaxOffset  = $section.offset().top;

			function update(){
				var scroll    = $(window).scrollTop(),
					winHeight = $(window).height(),
					opacity   = 2 * scroll / elementHeight,
					bgSize    = elementHeight + 300,
					posY      = ( scroll - ( parallaxOffset - winHeight ) ) / ( ( parallaxOffset + elementHeight ) - ( parallaxOffset - winHeight ) ),
					translate = posY * ( elementHeight - bgSize );
				$parallaxEl.css({ 'transform': 'translateY(' + translate + 'px)' , 'height': bgSize });
				$content.css({ 'opacity' : 1 - opacity });
			}
			update();
			...
			$(window).on("orientationchange",function(){
				elementHeight = GRVE.featureSize.init( section );
				$parallaxEl.css({ 'transform': 'translateY(0px)' });
				update();
			});
		}
	};
	...
	GRVE.pageSettings = {
		...
		init: function(){
			...
			this.backtoTop();
			this.animatedBg();
			this.menuWidget( '.widget_nav_menu');
			this.menuWidget( '.widget_pages');
			this.menuWidget( '#grve-hidden-menu .grve-menu');
			this.hovers();
			this.sideMenu();
		...
		bodyLoader: function(){
			...
			images.imagesLoaded(function(){
				setTimeout(function () {
					$loader.fadeOut(500);
					$overflow.delay(500).fadeOut(700);
				}, 600);
			});
		...
		grveModal: function(){
			...
			function modalHeight(){
				var $modalContent = $(content).find('.grve-modal-content'),
					modalHeight   = $(content).outerHeight(),
					windowHeight  = $(window).height();
				if( modalHeight > windowHeight ) {
					$(content).addClass('mobile');
					$themeWrapper.css({ 'height' : modalHeight, 'overflow':'hidden' });
				}
		...
		addSpinner: function( $selector ){
			...
			var $section = $selector,
				$spinner = $('<div class="grve-spinner"></div>');
		...
		stickySidebar: function(){
			...
			if( ( $(window).scrollTop() > contentTop - offset + 80 ) && ( $(window).scrollTop() < contentBottom - ( offset + itemHeight ) )){
				...
				$itemWrapper.css({'position':'absolute', 'top': contentHeight - itemHeight });
			}
		...
		hiddenArea: function( section, btn ){
			...
			$btn.on('click',function(e){
				...
				if( content.indexOf("#") === 0 && $(content).length > 0 ) {
					...
					GRVE.pageSettings.hiddenAreaHeight( content );
		...
		backtoTop: function() {
			...
			var selectors  = {
				...
				if( 1 == grve_main_data.grve_back_top_top ) {
					// Show backtoTop Button
					var $btnUp = $('<div class="grve-back-top"><i class="grve-icon-arrow-top-alt"></i></div>');
			...
			function toggle( $this, open ){
				...
				if( open ) {
					$subMenu.slideUp(200,function(){
						if( $menu.parent().is('.grve-scroller') ) {

							// Set Hidden Area Height
							var $parentArea = $menu.parents('.grve-hidden-area');
							GRVE.pageSettings.hiddenAreaHeight( $parentArea );
						}
					});
				} else {
		...
		sideMenu: function(){
			...
			var $sideArea = $('#grve-main-header.grve-header-side'),
				$menu     = $('#grve-main-menu.grve-vertical-menu'),
				$menuItem = $menu.find('li.menu-item-has-children > a'),
				$scroller = $sideArea.find('.grve-scroller'),
				$arrow    = $('<i class="grve-arrow"></i>');
		...
		sideMenuHeight: function(){
			var $sideArea    = $('#grve-main-header.grve-header-side'),
				$headerElem  = $sideArea.find('.grve-header-elements-wrapper'),
				$scroller    = $sideArea.find('.grve-scroller'),
				elHeight     = $headerElem.height(),
				windowHeight = $(window).height(),
				offset       = windowHeight - elHeight - 50;

				$scroller.css({ 'height' : offset }).perfectScrollbar();
		},
		...
		anchorSticky: function(){
			...
			var $anchorWrapper = $anchor.find('.grve-anchor-wrapper'),
				...
				headerHeight   = GRVE.header.calHeight(),
			...
			if( $anchor.hasClass('grve-current-link') ) {
				// findActive();
			}
		...
		shoppingCart: function(){
			...
			$cartList.perfectScrollbar();

			$button.on('mouseover',function(){
				clearTimeout(timer);
				$cartList.perfectScrollbar('update');
				openCart();
			});
		...
		init: function(){
			...
			function fullHeight( section ) {
				section.imagesLoaded('always',function(){
					...
					var windowHeight    = $(window).height(),
						sectionHeight   = GRVE.setColumnHeight.getMaxHeight( section ),
						headerHeight    = $('#grve-header').data('sticky') == 'none' ? $('#grve-main-header').outerHeight() : 0,
						topBarHeight    = $('#grve-top-bar').length ? $('#grve-top-bar').outerHeight() : 0,
						anchorBarHeight = $('.grve-anchor-menu').length ? $('.grve-anchor-menu').outerHeight() : 0,
	...
	GRVE.isotope = {
		...
		init: function(){
			...
			function itemSize( el, $container, callback ){
				var wrapperW     = el.innerWidth(),
					gap          = el.hasClass('grve-with-gap') ? 20 : 0,
					columns      = column( el ),
				...
				// Item Column 1
				if( columns == 1 ) {
					$container.css({ 'width' : columnW });
					$isotopItem.css({ 'width' : columnW });
		...
		// 			$isotopItem  = $container.find('.grve-isotope-item'),
		// 			$curCategory = $that.find('.grve-current-category'),
		// 			layout       = $that.data('layout') !== '' ? $that.data('layout') : 'fitRows',
		// 			dataSpinner  = $that.data('spinner');

		// 		if( dataSpinner == 'yes' ) {
		// 			GRVE.isotope.spinner( $that );
		// 		}
		// 		GRVE.isotope.isotopeInit( $container, layout, dataSpinner );
		...
		// 		if( dataSpinner == 'yes' ) {
		// 			setTimeout(function() {
		// 				GRVE.isotope.removeSpinner( $container );
		// 			},2000);
		// 		} else {
		// 			$container.animate({'opacity': 1},600);
		// 		}

		// 		GRVE.isotope.relayout( $container );
		...
		// 		var $that = $(this),
		// 			$container = $that.find('.grve-isotope-container'),
		// 			$element   = $that.find('.grve-isotope-item'),
		// 			$slider    = $that.find('.grve-slider'),
		// 			columns    = GRVE.isotope.defineColumns( $that ),
		// 			gap        = 0;

		// 		if( $that.parents('.grve-section').data('section-type') == 'fullwidth-element' && $that.hasClass('grve-with-gap') ) {
		// 			gap = - 40;
		// 		} else if( $that.parents('.grve-section').data('section-type') != 'fullwidth-element' && $that.hasClass('grve-with-gap') ) {
		// 			gap = 40;
		// 		}

		// 		var elementW = Math.floor(( $that.width() + gap - 0.5 ) / columns );

		// 		$element.css({ 'width': elementW });

		// 		// Item Width * 2
		// 		if( columns != 1 ) {
		// 			$that.find('.grve-image-large-square').css({ 'width': elementW * 2 });
		// 			$that.find('.grve-image-landscape').css({ 'width': elementW * 2 }).find('.grve-media').css({ 'height': elementW - Math.abs(gap) });
		// 			$that.find('.grve-image-portrait').css({ 'width': elementW }).find('.grve-media').css({ 'height': ( elementW * 2 ) - Math.abs(gap) });
		// 		}

		// 		// Item Column 2
		// 		if( columns == 2 ) {
		// 			$that.find('.grve-image-large-square').css({ 'width': elementW * 2 });
		// 			$that.find('.grve-image-landscape').css({ 'width': elementW  }).find('.grve-media').css({ 'height': ( elementW / 2 ) - Math.abs(gap) });
		// 			$that.find('.grve-image-portrait').css({ 'width': elementW }).find('.grve-media').css({ 'height': ( elementW * 2 ) - Math.abs(gap) });
		// 		}

		// 		// Item Column 1
		// 		if( columns == 1 ) {
		// 			$that.find('.grve-image-large-square').css({ 'width': elementW });
		// 			$that.find('.grve-image-landscape').css({ 'width': elementW  }).find('.grve-media').css({ 'height': ( elementW / 2 ) - Math.abs(gap) });
		// 			$that.find('.grve-image-portrait').css({ 'width': elementW }).find('.grve-media').css({ 'height': elementW - Math.abs(gap) });
		// 		}
